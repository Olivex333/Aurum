  ...
  app.post('/api/products', async (req, res) => {
    try {
      // Wyciągamy name, price, colors (obowiązkowe), resztę (opcjonalne)
      const {
        name,
        price,
        colors,
        sizes,        // nowa kolumna
        image_url,    // nowa kolumna
        description,  // nowa kolumna
      } = req.body;
      ...
      const values = [
        ...
        price,
        colors,
        sizes || null,       // jeśli puste, wstaw NULL
        image_url || null,   // jeśli puste, wstaw NULL
        description || null, // jeśli puste, wstaw NULL
      ];
  ...
  /** [Stary] GET /products - tu już niekoniecznie potrzebny, ale możesz zostawić */
  ...
  /** Tworzenie nowego usera (bez hasła) */
  ...
  /** [Opcjonalne] Rejestracja i logowanie */
...
app.post('/api/cart', verifyToken, async (req, res) => {
  try {
    ...
    // Sprawdzamy, czy w koszyku jest już taki sam produkt
    // o tym samym kolorze i rozmiarze:
    const checkQuery = `
      SELECT id, quantity
      FROM carts
      WHERE user_id = $1
    ...
    if (checkResult.rows.length > 0) {
      // Już istnieje w koszyku -> update quantity
      ...
      // Wstawiamy nowy wiersz z chosen_color, chosen_size
      const insertQuery = `
...
app.delete('/api/cart/:cartItemId', verifyToken, async (req, res) => {
  try {
    ...
    // Usuwamy TYLKO jeśli dany wiersz należy do zalogowanego usera
...
// Dodawanie do wishlist
app.post('/api/wishlist', verifyToken, async (req, res) => {
  try {
    const userId = req.user.userId
    const { productId } = req.body

    // Sprawdź czy już w wishlist
    const checkQuery = `
